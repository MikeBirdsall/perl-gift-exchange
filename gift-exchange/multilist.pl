#!/usr/local/bin/perl
# $Id: multilist.pl,v 1.8 2002/09/20 21:47:55 mbirdsal Exp $

# This CGI-BIN perl script is user script for Wish lists which operates on 
# multiple wishlists at the same time..
# 
...
my $gltitle = 'Hello '
  . display_name($gluser)
  . '! You Are On the Page To View Multiple Wish Lists';
...
# Choose the owners to display lists for if the user has explicitely asked to 
# choose them, or if they are not already chosen.
# paint_owner_choice_form does not return, but re-enters with param('owner') set
# as an array to multiple owners

if (   ( !param('owner') )
    or ( param('owner')  eq "" )
    or ( param('action') eq 'Pick Another List' ) )
{
    paint_owner_choice_form($gluser);
}

if ($debug) { print h2( { -class => 'Debug' }, param('owner') ) }

paint_wishlist($gluser);
paint_style_menu();
print end_form();
print end_html;
...
sub paint_wishlist
{
    ...
    paint_footer( undef, "Pick Another List" );
}
